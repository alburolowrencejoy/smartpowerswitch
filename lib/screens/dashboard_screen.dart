...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  void _listenToBuildings() {
    _buildingsSub = FirebaseDatabase.instance.ref('buildings').onValue.listen((event) {
      ...
      if (raw == null) { setState(() => _buildings = []); return; }
  ...
  void _listenToMasterDevices() {
    _masterSub = FirebaseDatabase.instance.ref('master_devices').onValue.listen((event) {
      if (!mounted) return;
      final raw = event.snapshot.value;
      if (raw == null) {
        setState(() { _assignedDevices = 0; _unassignedDevices = 0; _buildingDeviceCounts = {}; });
        return;
      }
      final data = Map<String, dynamic>.from(raw as Map);
      ...
      data.forEach((id, val) {
        ...
        if (assignedTo.isNotEmpty) {
          ...
          if (parts.isNotEmpty) bCounts[parts[0]] = (bCounts[parts[0]] ?? 0) + 1;
        } else { unassigned++; }
      });
      setState(() { _assignedDevices = assigned; _unassignedDevices = unassigned; _buildingDeviceCounts = bCounts; });
    }, onError: (Object error) { if (!mounted || _isPermissionDenied(error)) return; });
  }
  ...
  void _listenToEnergyData() {
    _devicesSub = FirebaseDatabase.instance.ref('devices').onValue.listen((event) {
      ...
      final raw = event.snapshot.value;
      if (raw == null) {
        setState(() { _totalKwh = 0; _totalCostPhp = 0; _buildingEnergy = {}; _utilityTotals = {}; });
        return;
      }
      final data = Map<String, dynamic>.from(raw as Map);
      double totalKwh = 0;
      ...
      data.forEach((id, val) {
        ...
        if (building.isNotEmpty) { bEnergy[building] = (bEnergy[building] ?? 0) + kwh.toDouble(); totalKwh += kwh.toDouble(); }
        final n = _capitalizeFirst(utility);
        if (n.isNotEmpty) uTotals[n] = (uTotals[n] ?? 0) + kwh.toDouble();
      });
      setState(() { _totalKwh = totalKwh; _totalCostPhp = totalKwh * _electricityRate; _buildingEnergy = bEnergy; _utilityTotals = uTotals; });
    }, onError: (Object error) { if (!mounted || _isPermissionDenied(error)) return; });
  }

  void _listenToRate() {
    _rateSub = FirebaseDatabase.instance.ref('settings/electricityRate').onValue.listen((event) {
      if (!mounted) return;
  ...
  void _listenToHistory() {
    _historySub?.cancel();
    _historySub = FirebaseDatabase.instance.ref('history/$_analyticsRange').onValue.listen((event) {
      if (!mounted) return;
      final raw = event.snapshot.value;
      ...
      data.forEach((key, val) {
        ...
        list.add({'label': key, 'kwh': (entry['total_kwh'] ?? 0.0) as num, 'cost': (entry['total_cost'] ?? 0.0) as num});
      });
  ...
  double get _maxKwh => _historyData.isEmpty ? 1
      : _historyData.fold(0.0, (m, d) => (d['kwh'] as num).toDouble() > m ? (d['kwh'] as num).toDouble() : m);
  ...
  String _capitalizeFirst(String s) => s.isEmpty ? s : s[0].toUpperCase() + s.substring(1).toLowerCase();
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (code.isEmpty)  { setS(() => error = 'Code is required'); return; }
                if (name.isEmpty)  { setS(() => error = 'Name is required'); return; }
                if (floors < 1)    { setS(() => error = 'Floors must be at least 1'); return; }
                // Check duplicate
                final existing = _buildings.any((b) => b['code'] == code);
                if (existing) { setS(() => error = 'Building $code already exists'); return; }
                ...
                try {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('$code added.')));
                } catch (e) {
                  setS(() => error = 'Failed: $e');
                }
              },
  ...
  Future<void> _deleteBuilding(Map<String, dynamic> building) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('Delete Building',
            style: TextStyle(fontFamily: 'Outfit', fontWeight: FontWeight.w600)),
        content: Text('Delete "$code"? This will also remove its hotspot from the map.'),
        actions: [
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$code removed.')));
  }
            ...
            Padding(
              ...
              child: Row(children: [
                const Text('Manage Buildings',
                    style: TextStyle(fontFamily: 'Outfit', fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.textDark)),
                const Spacer(),
                ElevatedButton.icon(
                  onPressed: () async {
                    Navigator.pop(ctx);
                        ...
                        return Container(
                          ...
                          child: Row(children: [
                            ...
                            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Text(b['name'], style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textDark)),
                              Text('${b['floors']} ${b['floors'] == 1 ? 'floor' : 'floors'}',
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          return Row(children: [
            ...
            const Expanded(
              child: Text('SmartPowerSwitch', maxLines: 1, overflow: TextOverflow.ellipsis,
            ...
            if (!isCompact) ...[
              Container(
                ...
                decoration: BoxDecoration(
                  color: _role == 'admin' ? AppColors.greenLight.withAlpha(51) : Colors.white.withAlpha(26),
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(color: _role == 'admin' ? AppColors.greenLight.withAlpha(102) : Colors.white.withAlpha(51)),
                ),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(_role == 'admin' ? Icons.star : Icons.person, size: 11,
              ...
              if (_role == 'admin')
                IconButton(icon: const Icon(Icons.settings_outlined, color: Colors.white, size: 22),
              ...
              PopupMenuButton<String>(
                ...
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14),
                    side: BorderSide(color: AppColors.greenLight.withAlpha(140))),
                onOpened: () { if (mounted) setState(() => _compactMenuOpen = true); },
                icon: AnimatedContainer(
                  duration: const Duration(milliseconds: 220),
                  curve: Curves.easeOut,
                  width: 36, height: 36,
                  decoration: BoxDecoration(
                    color: _compactMenuOpen ? AppColors.greenLight.withAlpha(46) : Colors.white.withAlpha(20),
                  ...
                  child: AnimatedRotation(
                    ...
                    duration: const Duration(milliseconds: 220),
                    curve: Curves.easeOut,
                    child: Icon(_compactMenuOpen ? Icons.close_rounded : Icons.menu_rounded,
                        color: Colors.white, size: 20),
                  ),
                ),
                onCanceled: () { if (mounted) setState(() => _compactMenuOpen = false); },
                onSelected: (value) {
                ...
                itemBuilder: (context) => [
                  const PopupMenuItem<String>(value: 'notifications',
                      child: Row(children: [Icon(Icons.notifications_outlined, size: 18, color: Colors.white), SizedBox(width: 10), Text('Notifications', style: TextStyle(color: Colors.white))])),
                  if (_role == 'admin')
                    const PopupMenuItem<String>(value: 'settings',
                        child: Row(children: [Icon(Icons.settings_outlined, size: 18, color: Colors.white), SizedBox(width: 10), Text('Settings', style: TextStyle(color: Colors.white))])),
                  const PopupMenuItem<String>(value: 'logout',
                      child: Row(children: [Icon(Icons.logout, size: 18, color: Colors.white), SizedBox(width: 10), Text('Logout', style: TextStyle(color: Colors.white))])),
                ],
              ),
          ]);
        },
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_role == 'admin')
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Icon(Icons.settings, size: 16, color: AppColors.greenDark),
              ),
            ),
          const Spacer(),
        ...
        if (_buildings.isEmpty)
          Container(
            ...
            child: Center(
              child: Column(children: [
                const Icon(Icons.business_outlined, size: 32, color: AppColors.textMuted),
                const SizedBox(height: 8),
                const Text('No buildings yet', style: TextStyle(fontSize: 13, color: AppColors.textMuted)),
                if (_role == 'admin') ...[
                  const SizedBox(height: 8),
                  TextButton.icon(
    ...
    return Row(children: [
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(_userName.isNotEmpty ? _userName : 'User',
            style: const TextStyle(fontFamily: 'Outfit', fontSize: 22, fontWeight: FontWeight.w700, color: AppColors.textDark)),
      ])),
      Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Container(width: 7, height: 7, decoration: const BoxDecoration(color: AppColors.greenMid, shape: BoxShape.circle)),
          const SizedBox(width: 5),
          const Text('Live', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: AppColors.greenDark)),
        ]),
      ),
    ]);
  }
  ...
  Widget _buildEnergyCards() {
    return Column(children: [
      Container(
        ...
        decoration: BoxDecoration(
          gradient: const LinearGradient(colors: [AppColors.greenDark, Color(0xFF1E7A42)],
              begin: Alignment.topLeft, end: Alignment.bottomRight),
          borderRadius: BorderRadius.circular(22),
          boxShadow: [BoxShadow(color: AppColors.greenDark.withAlpha(77), blurRadius: 20, offset: const Offset(0, 8))],
        ),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Total Energy Today', style: TextStyle(fontSize: 12, color: Color(0xB3C2EDD0))),
          const SizedBox(height: 6),
          Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text(_totalKwh.toStringAsFixed(1),
                style: const TextStyle(fontFamily: 'Outfit', fontSize: 40, fontWeight: FontWeight.w700, color: Colors.white)),
            const Padding(padding: EdgeInsets.only(bottom: 6, left: 6),
                child: Text('kWh', style: TextStyle(fontSize: 14, color: AppColors.greenLight, fontWeight: FontWeight.w500))),
          ]),
      ...
      Row(children: [
        ...
        const SizedBox(width: 10),
        Expanded(child: _statCard('Power Factor', '0.98',  Icons.speed,               AppColors.greenPale, textColor: AppColors.greenDark)),
      ]),
    ]);
  }

  Widget _vertDivider() => Container(width: 1, height: 28, margin: const EdgeInsets.symmetric(horizontal: 10), color: Colors.white.withAlpha(30));
  ...
  Widget _statCard(String label, String value, IconData icon, Color bg, {Color textColor = Colors.white}) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 8),
        Text(value, style: TextStyle(fontFamily: 'Outfit', fontSize: 15, fontWeight: FontWeight.w700, color: textColor)),
  ...
  Widget _buildBuildingCard(Map<String, dynamic> building) {
    ...
    return GestureDetector(
      onTap: () => Navigator.pushNamed(context, '/building', arguments: {
        'buildingCode': code, 'buildingName': building['name'],
        'floors': building['floors'], 'role': _role,
      }),
      child: Container(
        ...
        child: Row(children: [
          Container(width: 44, height: 44,
              ...
              child: Center(child: Text(code,
                  style: const TextStyle(fontFamily: 'Outfit', fontSize: 10, fontWeight: FontWeight.w700, color: AppColors.greenDark)))),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(building['name'], style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textDark)),
            const SizedBox(height: 2),
            Text('${building['floors']} ${building['floors'] == 1 ? 'floor' : 'floors'} · $devices devices',
                style: const TextStyle(fontSize: 11, color: AppColors.textMuted)),
          ])),
  ...
  Widget _buildAnalyticsTab() {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(20),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Analytics', style: TextStyle(fontFamily: 'Outfit', fontSize: 22, fontWeight: FontWeight.w700, color: AppColors.textDark)),
        const SizedBox(height: 4),
        const Text('Realtime energy insights', style: TextStyle(fontSize: 12, color: AppColors.textMuted)),
        const SizedBox(height: 20),
        _buildRangeSelector(),
        const SizedBox(height: 20),
        _buildLineChart(),
        ...
        SizedBox(
          ...
          child: OutlinedButton.icon(
            ...
            style: OutlinedButton.styleFrom(foregroundColor: AppColors.greenDark,
    ...
    return Container(
      ...
      child: Row(children: ranges.map((r) {
        ...
        return Expanded(child: GestureDetector(
          ...
          child: AnimatedContainer(duration: const Duration(milliseconds: 200), margin: const EdgeInsets.all(4),
            decoration: BoxDecoration(color: isSelected ? AppColors.greenDark : Colors.transparent, borderRadius: BorderRadius.circular(8)),
            child: Center(child: Text(r['label']!, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600,
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Consumption Trend', style: TextStyle(fontFamily: 'Outfit', fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textDark)),
            SizedBox(height: 2),
            Text('kWh over time · realtime', style: TextStyle(fontSize: 11, color: AppColors.textMuted)),
          ])),
          Container(width: 8, height: 8, decoration: const BoxDecoration(color: AppColors.greenMid, shape: BoxShape.circle)),
          const SizedBox(width: 5),
          const Text('Live', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.greenMid)),
        ]),
        const SizedBox(height: 20),
        _historyData.isEmpty
            ? const Center(child: Padding(padding: EdgeInsets.symmetric(vertical: 40),
                child: Text('No data yet', style: TextStyle(fontSize: 13, color: AppColors.textMuted))))
        ...
        if (_historyData.isNotEmpty) ...[
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text(_historyData.first['label'], style: const TextStyle(fontSize: 9, color: AppColors.textMuted)),
            if (_historyData.length > 2) Text(_historyData[_historyData.length ~/ 2]['label'],
                style: const TextStyle(fontSize: 9, color: AppColors.textMuted)),
            Text(_historyData.last['label'], style: const TextStyle(fontSize: 9, color: AppColors.textMuted)),
          ]),
        ],
      ]),
    );
  ...
  Widget _buildDeviceStatusCard() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: AppColors.cardBg, borderRadius: BorderRadius.circular(18),
          border: Border.all(color: AppColors.greenMid.withAlpha(26))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Device Status', style: TextStyle(fontFamily: 'Outfit', fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textDark)),
        const SizedBox(height: 16),
        Row(children: [
        ...
        ClipRRect(borderRadius: BorderRadius.circular(6),
            child: LinearProgressIndicator(value: assignedPct, minHeight: 8,
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('$count', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: color, fontFamily: 'Outfit')),
  ...
  Widget _buildTopUtilityCard() {
    ...
    final sorted = _utilityTotals.entries.toList()..sort((a, b) => b.value.compareTo(a.value));
    final maxVal = sorted.first.value;
    final Map<String, Color> colors = {'Lights': AppColors.greenMid, 'Outlets': const Color(0xFFE8922A), 'AC': const Color(0xFF2196F3)};
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(color: AppColors.cardBg, borderRadius: BorderRadius.circular(18),
          border: Border.all(color: AppColors.greenMid.withAlpha(26))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Top Consuming Utilities', style: TextStyle(fontFamily: 'Outfit', fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textDark)),
        const SizedBox(height: 16),
        ...sorted.map((e) {
          ...
          return Padding(padding: const EdgeInsets.only(bottom: 14),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 6),
              ClipRRect(borderRadius: BorderRadius.circular(6),
                  child: LinearProgressIndicator(value: pct, minHeight: 7, backgroundColor: color.withAlpha(25), valueColor: AlwaysStoppedAnimation<Color>(color))),
            ]));
        }),
      ]),
    );
  ...
  Widget _buildTopBuildingCard() {
    ...
    final sorted = _buildingDeviceCounts.entries.toList()..sort((a, b) => b.value.compareTo(a.value));
    final maxVal = sorted.first.value;
    final List<Color> barColors = [AppColors.greenDark, AppColors.greenMid, AppColors.greenLight, const Color(0xFF2196F3), const Color(0xFFE8922A)];
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(color: AppColors.cardBg, borderRadius: BorderRadius.circular(18),
          border: Border.all(color: AppColors.greenMid.withAlpha(26))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Devices per Institute', style: TextStyle(fontFamily: 'Outfit', fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textDark)),
        const SizedBox(height: 16),
        ...sorted.asMap().entries.map((entry) {
          ...
          return Padding(padding: const EdgeInsets.only(bottom: 14), child: Row(children: [
            Container(width: 24, height: 24,
                decoration: BoxDecoration(color: i == 0 ? AppColors.greenDark : AppColors.greenPale, shape: BoxShape.circle),
                child: Center(child: Text('${i + 1}', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700,
                    color: i == 0 ? Colors.white : AppColors.greenDark)))),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text(e.key, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: AppColors.textDark)),
                Text('${e.value} devices', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.greenDark)),
              ]),
    ...
    return Container(
      ...
      child: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(icon: Icon(Icons.schedule_outlined),  activeIcon: Icon(Icons.schedule),  label: 'Automation'),
        ],
      ),
    );
  }
...
class _LineChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final linePaint = Paint()..color = AppColors.greenMid..strokeWidth = 2.5..style = PaintingStyle.stroke..strokeCap = StrokeCap.round..strokeJoin = StrokeJoin.round;
    ...
    Offset off(int i) => Offset(i * stepX, (size.height - (data[i] / maxKwh) * size.height).clamp(0.0, size.height));
    final gridPaint = Paint()..color = AppColors.greenMid.withAlpha(20)..strokeWidth = 1;
    for (int i = 1; i < 4; i++) {
      canvas.drawLine(Offset(0, size.height * i / 4), Offset(size.width, size.height * i / 4), gridPaint);
    }
    final fillPath = Path()..moveTo(0, size.height)..lineTo(off(0).dx, off(0).dy);
    for (int i = 1; i < data.length; i++) { final p = off(i-1); final c = off(i); fillPath.cubicTo((p.dx+c.dx)/2, p.dy, (p.dx+c.dx)/2, c.dy, c.dx, c.dy); }
    fillPath
      ..lineTo(size.width, size.height)
      ..close();
    canvas.drawPath(fillPath, fillPaint);
    final linePath = Path()..moveTo(off(0).dx, off(0).dy);
    for (int i = 1; i < data.length; i++) { final p = off(i-1); final c = off(i); linePath.cubicTo((p.dx+c.dx)/2, p.dy, (p.dx+c.dx)/2, c.dy, c.dx, c.dy); }
    canvas.drawPath(linePath, linePaint);
    for (int i = 0; i < data.length; i++) {
      canvas.drawCircle(off(i), 3.5, Paint()..color = AppColors.greenDark..style = PaintingStyle.fill);
      canvas.drawCircle(off(i), 3.5, Paint()..color = Colors.white..style = PaintingStyle.stroke..strokeWidth = 1.5);
    }
  }

  @override
  bool shouldRepaint(_LineChartPainter old) => old.data != data || old.maxKwh != maxKwh;
}